...
- Receive external webhook events (GitHub push, pull_request, etc.) and create Workflow CRs automatically
- Support parallel job grouping with `needs` dependencies
- Validate webhook payloads via HMAC signature
...
- Expose internal cluster details to external webhook callers
...
- Support parameter extraction from webhook payloads (e.g., branch name → env var)
- Provide Ingress manifests for webhook exposure
- Support workflow template reuse (EventTrigger → template Workflow CR)
...
- Require TLS in the webhook server itself — terminate at Ingress
